...
export default class PedidoService extends BackendClient<Pedido> {
  async buscarXSucursal(
    ...
    const response = await fetch(
      `${this.baseUrl}/buscar/${idSucursal}?${busquedaParam}&${desdeParam}&${hastaParam}&${estadoParam}`,
      {
  ...
  async getExcelPedidos(
    ...
    a.href = urlBlob;
    a.download = `pedidos${desde ? "-" + desde : ""}${
      hasta ? "-" + hasta : ""
    }${estado ? "-estado_" + estado : ""}${busqueda ? "-busqueda" : ""}.xlsx`;
  ...
  async getRankingProductos(
    ...
    const response = await fetch(
      `${this.baseUrl}/rankingproductos?idSucursal=${idSucursal}&${fechaDesdeParam}&${fechaHastaParam}`,
      { headers: this.getAuthHeaders() }
    );
    const data = await response.json();
    return data as [];
  ...
  async getExcelRankingProductos(
    idSucursal: number,
    desde?: string,
    hasta?: string
  ) {
  ...
  async getRankingClientes(
    ...
    desde: string,
    hasta: string,
    ordenarPorPedidos: boolean
  ): Promise<[]> {
    const limiteParam = limite ? `limite=${limite}` : "";
    const fechaDesdeParam = desde ? `desde=${desde}` : "";
    const fechaHastaParam = hasta ? `hasta=${hasta}` : "";
    const response = await fetch(
      `${this.baseUrl}/rankingclientes?${limiteParam}&idSucursal=${idSucursal}&${fechaDesdeParam}&${fechaHastaParam}&ordenarPorPedidos=${ordenarPorPedidos}`,
      { headers: this.getAuthHeaders() }
    );
    const data = await response.json();
    return data as [];
  ...
  async getExcelRankingClientes(
    ...
    const a = document.createElement("a");
    a.href = urlBlob;
    a.download = `ranking_clientes-${desde}-${hasta}-top_${limite}-ordenado_por_${
      ordenarPorPedidos ? "pedidos" : "monto"
    }.xlsx`; // Nombre del archivo descargado
    document.body.appendChild(a);
    a.click();
    a.remove();
    window.URL.revokeObjectURL(urlBlob);
  ...
  async getMovimientosMonetarios(
    idSucursal: number,
    desde: string,
    hasta: string,
    esDiario: boolean
    ...
    const fechaHastaParam = hasta ? `hasta=${hasta}` : "";
    const response = await fetch(
      `${this.baseUrl}/movimientosmonetarios?idSucursal=${idSucursal}&${fechaDesdeParam}&${fechaHastaParam}&esDiario=${esDiario}`,
      { headers: this.getAuthHeaders() }
    );
    const data = await response.json();
    return data as [];
  ...
  async getExcelMovimientosMonetarios(
    idSucursal: number,
    desde: string,
    hasta: string,
    esDiario: boolean
    ...
    const a = document.createElement("a");
    a.href = urlBlob;
    a.download = `movimientos_monetarios-${desde}-${hasta}-${
      esDiario ? "agrupamiento_diario" : "agrupamiento_mensual"
    }.xlsx`; // Nombre del archivo descargado
    document.body.appendChild(a);
    a.click();
    a.remove();
  ...
  async cancelarPedido(pedidoId: number) {
    const url = `${this.baseUrl}/cancelarPedido/${pedidoId}`;

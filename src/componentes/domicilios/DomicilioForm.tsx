...
function DomicilioForm({
  ...
  const getLocalidadesRest = async (idProvincia?: number) => {
    ...
    const localidadesFiltradas = idProvincia
      ? localidades.filter((l) => l.provincia.id === idProvincia)
      : localidades;
    const localidadesOpciones = [
      { id: 0, label: "Seleccione una localidad" },
      ...localidadesFiltradas.map((l) => {
        return { id: l.id, label: l.nombre };
      }),
    ];
  ...
  const getProvinciasRest = async (idPais?: number) => {
    ...
    const provinciasFiltradas = idPais
      ? provincias.filter((p) => p.pais.id === idPais)
      : provincias;
    const provinciasOpciones = [
      { id: 0, label: "Seleccione una provincia" },
  ...
  const getPaisesRest = async () => {
    const paises = [
      { id: 0, label: "Seleccione un país" },
      ...(await paisService.getAll()).map((p) => {
        return { id: p.id, label: p.nombre };
      }),
    ];
    setPaises(paises);
  ...
  useEffect(() => {
    if (selectedPais.id !== 0) getProvinciasRest(selectedPais.id);
  }, [selectedPais]);

  useEffect(() => {
    if (selectedProvincia.id !== 0) getLocalidadesRest(selectedProvincia.id);
  }, [selectedProvincia]);
      ...
      <div className="mb-3 row">
        ...
        <div className="col-3">
          ...
          <input
            ...
            id="numero"
            className="form-control"
            value={domicilio.numero === 0 ? "" : domicilio.numero}
        ...
        <div className="col-3">
          ...
          <input
            ...
            id="cp"
            className="form-control"
            value={domicilio.cp === 0 ? "" : domicilio.cp}
      ...
      <div className="mb-3 row">
        ...
        <div className="col">
          <Autocomplete
            ...
            size="small"
            options={provincias}
            getOptionDisabled={(option) => option === provincias[0]}
            isOptionEqualToValue={(option, value) => option.id === value.id}
            value={{
              id: selectedProvincia.id,
              label: selectedProvincia.nombre,
            }}
            renderInput={(params) => (
      ...
      <div className="mb-3 row align-items-end">
        <div className="col">
          <Autocomplete
            ...
            size="small"
            options={localidades}
            getOptionDisabled={(option) => option === localidades[0]}
            isOptionEqualToValue={(option, value) => option.id === value.id}
            value={{
              id: selectedLocalidad.id,
              label: selectedLocalidad.nombre,
            }}
            renderInput={(params) => (

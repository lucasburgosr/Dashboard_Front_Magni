...
const Promociones = () => {
  ...
  const handleEliminarEnSucursal = async (promocionCambiada: Promocion) => {
    if (
      promocionCambiada.sucursales
        ?.map((s) => s.id)
        .includes(sucursalSeleccionada.id)
    )
      promocionCambiada.sucursales = [
        ...promocionCambiada.sucursales!.filter(
  ...
  const getPromocionesRest = useCallback(async () => {
    try {
      ...
      const promocionesFiltrados = (
        tipoSeleccionado
          ? datos.filter(
              (promocion) => promocion.tipoPromocion === tipoSeleccionado
            )
          : datos
      )
        .filter((promocion) =>
        ...
        .filter((promocion) => {
          const incluyeSucursal = promocion.sucursales?.some(
            (s) => s.id === sucursalSeleccionada.id
          );
  ...
  const handleChangeTipoPromocion = (e: ChangeEvent<HTMLSelectElement>) => {
    ...
    if (e.target.value !== "0") setTipoSeleccionado(tipoSeleccionado);
    else setTipoSeleccionado("");
  };
  ...
  const handleSave = useCallback(async () => {
    ...
    if (promocion.descripcionDescuento === "") {
      erroresNuevos["descripcionDescuento"] =
        "Debe ingresar una descripción del producto";
    }
    if (
      String(promocion.fechaDesde) === "" ||
      String(promocion.horaHasta).length > 10
    ) {
      erroresNuevos["fechaDesde"] =
        "Debe ingresar la fecha de inicio de la promoción";
    }
    if (
      String(promocion.fechaHasta) === "" ||
      String(promocion.horaHasta).length > 10
    ) {
      erroresNuevos["fechaHasta"] =
        "Debe ingresar la fecha de finalizado de la promoción ";
    }
    if (
      String(promocion.horaDesde) === "" ||
      String(promocion.horaHasta).length > 8
    ) {
      erroresNuevos["horaDesde"] =
        "Debe ingresar el horario de inicio de la promoción";
    }
    if (
      String(promocion.horaHasta) === "" ||
      String(promocion.horaHasta).length > 8
    ) {
      erroresNuevos["horaHasta"] =
        "Debe ingresar el horario de finalizado de la promoción";
    }
    if (promocion.fechaDesde >= promocion.fechaHasta) {
      if (promocion.fechaDesde > promocion.fechaHasta) {
        erroresNuevos["fechaHasta"] =
          "La fecha final no puede superar la fecha inicial";
      } else if (promocion.horaDesde > promocion.horaHasta) {
        erroresNuevos["horaHasta"] =
          "La hora final no puede superar la hora inicial del mismo día";
      }
    }
    if (promocion.precioPromocional < 0) {
      erroresNuevos["precioPromocional"] =
        "Debe ingresar el precio promocional que sea mayor o igual a cero";
    } else if (promocion.precioPromocional >= 1000000000) {
      erroresNuevos["precioPromocional"] =
        "El precio promocional no puede ser tan alto. limítese a 9 cifras";
    }
    ...
    if (promocion.promocionDetalles.length === 0) {
      erroresNuevos["promocionDetalles"] =
        "Debe ingresar al menos un artículo en la promoción";
    }
        ...
        <TableRow sx={{ "& > *": { borderBottom: "unset" } }}>
          ...
          <TableCell>
            {row.fechaDesde
              ...
              .reverse()
              .join("/")}{" "}
            {String(row.horaDesde).slice(0, 5)} -{" "}
            {row.fechaHasta
              .toLocaleString("es-AR")
              .split("-")
              .reverse()
              .join("/")}{" "}
            {String(row.horaHasta).slice(0, 5)}
          </TableCell>
          <TableCell align="right">
            ${row.precioPromocional.toLocaleString("es-AR")}
          </TableCell>
                    ...
                    {row.promocionDetalles?.map((promocionDetalle) => (
                      <TableRow key={promocionDetalle.id}>
                        <TableCell component="th" scope="row">
                          {promocionDetalle.articulo.denominacion}
                        </TableCell>
                        <TableCell align="right">
                          {promocionDetalle.cantidad!.toLocaleString("es-AR")}{" "}
                          {promocionDetalle.articulo.unidadMedida.denominacion}
                        </TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
  ...
  const emptyRows =
    rowsPerPage -
    Math.min(rowsPerPage, promociones.length - page * rowsPerPage);
        ...
        <div className="d-flex justify-content-between">
          ...
          <div className="col mb-3 mt-auto d-flex justify-content-end">
            <CboBoxFiltrar
              ...
              datos={[
                { id: 1, denominacion: "HappyHour" },
                { id: 2, denominacion: "Promocion" },
              ]}

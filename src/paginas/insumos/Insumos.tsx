...
const Insumos = () => {
  ...
  const [errors, setErrors] = useState<{
    [key in keyof ArticuloInsumo]?: string;
  }>({});
  ...
  const getInsumosRest = async () => {
    const datos: ArticuloInsumo[] =
      await articuloInsumoService.buscarXDenominacion(busqueda);
    datos.forEach((articulo) => {
      const stockSucursal = articulo.stocksInsumo?.find(
        (s) => s.sucursal.id === sucursalSeleccionada.id
      );
    ...
    const insumosFiltrados = (
      categoriaSeleccionada
        ? datos.filter(
            (insumo) => insumo.categoria.id === categoriaSeleccionada
          )
        : datos
    ).filter((i: ArticuloInsumo) => {
      const incluyeSucursal = i.stocksInsumo?.some(
        (s) => s.sucursal.id === sucursalSeleccionada.id
      );
  ...
  const handleEliminarEnSucursal = async (articuloInsumo: ArticuloInsumo) => {
    const stockSucursalActual =
      articuloInsumo.stocksInsumo!.find(
        (s) => s.sucursal.id === Number(sucursalSeleccionada.id)
      ) || new StockInsumo();
    if (!stockSucursalActual.sucursal.id) {
      stockSucursalActual.sucursal = sucursalSeleccionada;
      articuloInsumo.stocksInsumo = [
        ...articuloInsumo.stocksInsumo!,
        stockSucursalActual,
      ];
    } else {
      articuloInsumo.stocksInsumo = articuloInsumo.stocksInsumo!.filter(
        (s) => s.id !== stockSucursalActual.id
      );
    }
  ...
  const handleSave = useCallback(async () => {
    ...
    if (articuloInsumo.stockMinimo! < 0) {
      erroresNuevos["stockMinimo"] =
        "Debe ingresar un stock mínimo válido, que sea mayor o igual a cero.";
    } else if (articuloInsumo.stockMinimo! >= 1000000000) {
      erroresNuevos["stockMinimo"] =
        "El stock mínimo es demasiado grande. limítese a 9 cifras";
    }
    if (articuloInsumo.stockMaximo! < 0) {
      erroresNuevos["stockMaximo"] =
        "Debe ingresar un stock máximo válido, que sea mayor o igual a cero.";
    } else if (articuloInsumo.stockMaximo! >= 1000000000) {
      erroresNuevos["stockMaximo"] =
        "El stock máximo es demasiado grande. limítese a 9 cifras";
    ...
    if (articuloInsumo.stockActual! < 0) {
      erroresNuevos["stockActual"] =
        "Debe ingresar un stock válido, que sea mayor o igual a cero";
    } else if (articuloInsumo.stockActual! >= 1000000000) {
      erroresNuevos["stockActual"] =
        "El stock actual es demasiado grande. limítese a 9 cifras";
    } else {
    ...
    if (articuloInsumo.precioCompra! < 0) {
      erroresNuevos["precioCompra"] =
        "Debe ingresar un precio de compra válido, que sea mayor a cero.";
    } else if (articuloInsumo.precioCompra >= 1000000000) {
      erroresNuevos["precioCompra"] =
        "El precio de compra es demasiado grande. limítese a 9 cifras";
    }
    if (articuloInsumo.precioVenta! < 0) {
      erroresNuevos["precioVenta"] =
        "Debe ingresar un precio de venta válido, que sea mayor o igual a cero.";
    ...
    if (
      Object.keys(erroresNuevos).some((key) => {
    ...
    if (articuloInsumo.id === 0) {
      ...
      const stockSucursalActual = articuloInsumo.stocksInsumo!.find(
        (s) => s.sucursal.id === Number(sucursalSeleccionada.id)
      );
      ...
      articuloInsumo.stocksInsumo = [
        ...articuloInsumo.stocksInsumo!.filter(
          (s) => s.id !== stockSucursalActual.id
        ),
        stockSucursalActual!,
      ];
  ...
  useEffect(() => {
    setCategoriasFiltradas(
      categorias.filter((categoria) =>
        insumos.some((insumo) => insumo.categoria.id === categoria.id)
      )
    );
  }, [categorias]);
      ...
      <React.Fragment>
        <TableRow sx={{ "& > *": { borderBottom: "unset" } }}>
          ...
          <TableCell align="center">
            <a
              className={
                "my-auto btn " + (row.esParaElaborar ? "" : "btn-dark")
              }
              style={{
                width: "100px",
                marginBottom: 10,
                backgroundColor: row.esParaElaborar ? "#a6c732" : "",
                color: row.esParaElaborar ? "white" : "",
              }}
              onClick={() => {
                handleElaborarChange(row);
              }}
            >
              {row.esParaElaborar ? "Sí" : "No"}
            </a>
          </TableCell>
            ...
            <div className="d-flex justify-content-end">
              {mostrarVisibles ? (
                <>
                  ...
                  <div className="ms-2" />
                  <BtnDelete
                    handleClick={() => handleEliminarEnSucursal(row)}
                  />
                </>
              ) : (
                <BtnAdd
                  ...
                  <TableBody>
                    {row.stocksInsumo!.map((stocksInsumo) => (
                      <TableRow key={stocksInsumo.id}>
                        ...
                        <TableCell align="right">
                          {stocksInsumo.stockActual!.toLocaleString("es-AR")}{" "}
                          {row.unidadMedida.denominacion}
                        </TableCell>
                        <TableCell align="right">
                          {stocksInsumo.stockMinimo!.toLocaleString("es-AR")}{" "}
                          {row.unidadMedida.denominacion}
                        </TableCell>
                        <TableCell align="right">
                          {stocksInsumo.stockMaximo!.toLocaleString("es-AR")}{" "}
                          {row.unidadMedida.denominacion}
                        </TableCell>
                      </TableRow>
                    ))}
  ...
  const emptyRows =
    rowsPerPage - Math.min(rowsPerPage, insumos.length - page * rowsPerPage);
    ...
    <div className="m-3">
      <Modal show={show} onHide={handleClose} className="modal-xl">
        ...
        <Modal.Body>
                ...
                <Form.Group className="mb-3" controlId="stockActual">
                  ...
                  <div className="input-group">
                    ...
                    <span className="input-group-text">
                      {
                        unidadesMedida?.find(
                          (u) => u.id === articuloInsumo.unidadMedida.id
                        )?.denominacion
                      }
                    </span>
                  </div>
              ...
              <div className="col-sm col-md col-lg d-flex flex-column justify-content-between">
                ...
                <Form.Group className="mb-3" controlId="unidadMedida">
                  ...
                  <div className="d-flex">
                    ...
                    <BtnAddCategory
                      openModal={() => setShowUnidadesMedida(true)}
                    />
                  </div>
                  {errors["unidadMedida"] && (
                    <div className="ms-1 mt-1 text-danger">
          ...
          {showCategorias && <div className="modal-backdrop fade show"></div>}
          <Modal
            show={showCategorias}
          ...
          <Modal
            show={showUnidadesMedida}
            onHide={() => setShowUnidadesMedida(false)}
          >
            <Modal.Header closeButton className="custom-modal-header">
              <Modal.Title className="custom-modal-title">
                Unidades de Medida
            ...
            <Modal.Footer>
              <Button
                className="custom-btn-enviar"
                variant="primary"
                onClick={() => setShowUnidadesMedida(false)}
              >
                Guardar
              </Button>
            </Modal.Footer>
